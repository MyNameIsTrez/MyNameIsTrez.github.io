// use `node parseMidi.js` with node in this folder to execute
...
const nps = 5; // the amount of times a single noteblock can make a sound in a second
...
for (let name in names) {
  ...
  for (let i = 0; i < midiArray.length; i++) {
    for (let j = 0; j < midiArray[i].length; j++) {
      ...
      midiArray[i][j] = [Math.round(midiArray[i][j].deltaTime / nps), [instruments[i]]];
      for (let k = 0; k < data.length; k++) {
        midiArray[i][j][1].push(Math.round(data[k] * (24 / 127)));
      }
    }
  }
