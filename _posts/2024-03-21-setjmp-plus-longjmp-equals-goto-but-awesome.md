...
static token get_token(size_t token_index) {
    if (token_index >= tokens.size) {
        ...
        error_happened = true;
        return (token){}; // We can't just do `return;` :(
    }
    return tokens.tokens[token_index];
}
```
...
This isn't a huge issue, but it's extremely hard to tell where I forgot to paste it. If I had forgotten to paste it in the above code block and an out-of-bounds index were passed to that `get_token()` call, `token.type` would have an undefined value, and could've caused a segfault at `get_token_type_str[token.type]` when its gets used as an index into an array.
