...
In this case however we're assuming that the hash table is only built once the entire array is never going to be modified again, so [profile.c](https://github.com/MyNameIsTrez/array-based-hash-table-in-c/blob/main/profile.c) just profiles how long it takes to figure out if a random element is present, in a hash table and a regular array:
