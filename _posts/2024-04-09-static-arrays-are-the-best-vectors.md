...
Another benefit of static arrays over vectors is that pointers to elements in the array don't start dangling once the array "grows". [This is a common bug](https://stackoverflow.com/a/8261069/13279557) in C and C++ code that uses `malloc()`/`realloc()`/`std::vector`.
