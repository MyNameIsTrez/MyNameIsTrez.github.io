...
The common explanation is that in C, whenever you pass something as a function call argument, it is always passed by value, and never by reference. What this means for this block of code is that while the `int x` argument in `by_value()` gets a value of 68, just like the `x` variable in `main()`, they are completely separate variables. So this means that modifying the `x` in `by_value()` doesn't change the value of the `x` in `main()`.
