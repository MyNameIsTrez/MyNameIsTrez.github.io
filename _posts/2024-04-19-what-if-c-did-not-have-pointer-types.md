...
The common explanation is that whenever you pass something as an argument into a function call in C, the argument is always passed by value. What this means is that with `by_value(x);` the value 68 is passed, whereas with `by_pointer(&x);` a value like 0x12345678 is passed.
