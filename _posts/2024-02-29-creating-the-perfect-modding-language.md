...
grug is able to be a stupidly simple language mainly because it doesn't allow mods to store their own global state, and because it only allows `on_` event handling functions to run on things `define_`d in the same file. grug is just a basic configuration and scripting language that allows modders to create simple functions that act directly on the game's state.
