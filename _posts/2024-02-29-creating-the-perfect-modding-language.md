...
So a modder might call `vector_string_create()`, which returns an `i32` ID, and is then used when calling `vector_string_push(id, "foo")` and `vector_string_get(id, index)`.
