...
And yes, Lua will throw an error when something like `print(marine.health + 42)` is done if the `health` field were be removed from the game's `human` struct, but this error is often only discovered when some poor sod playing with a gazillion mods accidentally triggers that line of code.
